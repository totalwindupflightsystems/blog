...
This isn't just about Fable 5. It's a broader industry shift. In April 2026, Anthropic unbundled its enterprise pricing — seat fees no longer include prepaid token allowances. Starting June 15, agent workloads (Claude Code, the Agent SDK, third-party app usage) were moved from flat-rate subscription pools to a separate metered credit system. The subscription that enterprises negotiated was replaced by a meter. The meter has no cap.

The result is already visible. **Uber burned through its entire 2026 AI coding tools budget in four months.** Not "trending over budget." Completely exhausted by April. The cost per engineer ranged from $500 to $2,000 per month — driven entirely by token-based pricing that scales with actual usage. Uber's COO is now publicly questioning whether the spending is worth it. They had incentivized adoption through an internal leaderboard ranking teams by AI tool usage. The meter ate the budget.

Uber is not alone. Enterprise AI costs are exploding across the industry. One analysis of 2.4 billion enterprise API calls found that while blended per-token costs dropped 67% year-over-year, *total* spend kept rising because usage volume grew faster than prices fell. The per-token price goes down. The meter spins faster. The bill goes up.
...
If Anthropic keeps Fable 5 at $10/$50 per million tokens with mandatory data retention, enterprises keep disabling it and moving to open-source alternatives. Several practitioners confirmed their companies are "actively trying to lower Anthropic model usage." The market is voting with procurement. Uber's budget was gone in four months. CEOs are asking their COOs whether AI spending is worth it. The per-token meter is producing revenue on paper and resentment in procurement departments.
...
And beneath all of it is Nvidia. In Q4 FY2026, Nvidia reported **75% gross margins** — a level, as ServeTheHome noted, "few chipmakers have ever reached." $43 billion in net income. In a single quarter. Every token an enterprise buys from Anthropic or OpenAI flows upstream: a slice to the AI lab, a slice to the cloud provider, and the biggest slice to the company that made the GPU. Nvidia doesn't care which lab wins. It doesn't care whether enterprises choose Claude or GPT or an open-source model hosted on Bedrock. Every path runs through its silicon.

This is the structural trap in full. The AI labs have committed hundreds of billions to cloud providers. The cloud providers have committed those billions to Nvidia. Nvidia takes 75 cents of every dollar. The labs can only sustain this by charging enterprises per-token prices that enterprises are already rebelling against. If the prices come down, the datacenters become stranded assets — hundreds of billions in GPUs running workloads that no longer justify the capital expenditure. If the prices stay up, the customers leave for models that don't participate in the loop.
