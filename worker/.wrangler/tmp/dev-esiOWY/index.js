...
// .wrangler/tmp/bundle-ChIpIE/strip-cf-connecting-ip-header.js
...
function renderArticlePage(slug, md) {
  ...
  return fill(SPA_SHELL, {
    ...
    OGTYPE: "article",
    OGIMAGE: ogImage,
    CONTENT: `${hero}<h1>${esc(title)}</h1>${dateLine}${html2}`
  });
}
...
function renderHomePage(manifest) {
  const items = (manifest.articles || []).map((a) => {
    ...
    return `<article class="article-card">
      ...
      <p class="post-meta">${d}${a.tags?.length ? " \xB7 " + a.tags.map((t) => `<span class="tag-pill">${esc(t)}</span>`).join(" ") : ""}</p>
...
__name(originFetch, "originFetch");
var src_default = {
  async fetch(request, env) {
    ...
    if (/\.(png|jpe?g|webp|gif|svg|css|js|xml|json|txt|ico|woff2?|md)$/i.test(path)) {
      const resp2 = await originFetch(path, request);
      const ext = path.match(/\.([a-z0-9]+)$/i)?.[1].toLowerCase();
      const ct = CONTENT_TYPES[`.${ext}`];
      const headers2 = cleanHeaders(resp2.headers);
      if (ct)
        headers2.set("content-type", ct);
      return new Response(resp2.body, { status: resp2.status, headers: headers2 });
    }
    if (isBot) {
      if (path === "/" || path === "") {
...
// .wrangler/tmp/bundle-ChIpIE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ChIpIE/middleware-loader.entry.ts
